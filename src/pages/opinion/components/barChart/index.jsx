		...
		function initData() {
			...
			for (let i = 0; i < days; i++) {
				...
				promiseArr.push(
					...
					//axios.post("http://172.81.247.83/opinion/getemotion_file",{"date2":date}).then((res) => {
						axios({	
							method:'post',
							url:"http://172.81.247.83/opinion/getemotion_file",
							data:Qs.stringify({
								"date2": date
							})
						}).then((res) => {
			...
			Promise.all(promiseArr).then(() => {
				...
				axios.get("http://172.81.247.83/opinion/getemotionum").then((res) => {
