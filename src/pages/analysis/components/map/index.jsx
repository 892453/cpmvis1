...
export default function Radar() {
	const timetest=new Date().toLocaleDateString() //今天的日期
	const [currentLevel, setCurrentLevel] = useState("province") //当前层级

	const [currentDate, setCurrentDate] = useState(moment(timetest).format("YYYY-MM-DD")) //直接展示当天的34个省份疫情数据
	...
	useEffect(() => {
		...
		axios("http://172.81.247.83/analysis/getprovincedatadaily").then(
			(res) => {
				let data = [] //热力图数据
				if (true) {
					res.map((item) => {
	...
	function disabledDate(current) {
		return (
			...
			|| current > moment(new Date(moment(timetest).format("YYYY/MM/DD")))
			//moment(timetest).format("YYYY-MM-DD")
		)
	}
